...
- **Load**: [Extending Extraction Guide]()
...
```
For persistent config, set `locking.concurrencyScope` in `vibe-validate.config.yaml`. See [Configuration Reference]() for all locking options.
