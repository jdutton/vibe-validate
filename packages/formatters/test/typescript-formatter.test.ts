...
import { formatTypeScriptErrors } from '../src/typescript-formatter.js';

describe('formatTypeScriptErrors', () => {
  it('should parse single TypeScript error', () => {
    const output = `
src/index.ts(10,5): error TS2322: Type 'string' is not assignable to type 'number'.
    `.trim();

    const result = formatTypeScriptErrors(output);
...
src/utils.ts(100,3): warning TS6133: 'unusedVar' is declared but never used.
  ...
  it('should limit output to first 10 errors', () => {
    ...
    const result = formatTypeScriptErrors(errors);
...
src/config.ts(25,12): error TS2304: Cannot find name 'process'.
  ...
  it('should handle empty output', () => {
    const result = formatTypeScriptErrors('');
