...
function extract(output: string): ErrorExtractorResult {
  ...
  /**
   * Parse TypeScript errors from output using given pattern
   */
  const parseErrors = (pattern: RegExp, output: string, errors: FormattedError[]): void => {
    let match;
  ...
  parseErrors(newPattern, output, errors);

  // Try old format if no matches yet
  if (errors.length === 0) {
    parseErrors(oldPattern, output, errors);
  }
