...
import { getRepositoryRoot, stripGitEnv } from '@vibe-validate/git';
import { safeExecSync, toForwardSlash } from '@vibe-validate/utils';
...
/**
 * Re-exported from `@vibe-validate/git`, where the hazard knowledge now lives.
 *
 * The blacklist is git-specific (which `GIT_*` vars redirect a child away from
 * the repository it was handed), so it belongs with the rest of this project's
 * git expertise rather than in the process-plumbing layer that happens to be its
 * busiest caller. `core` already depends on `git`, so the move is downward.
 *
 * @public
 */
export { stripGitEnv } from '@vibe-validate/git';
