 ...
 */
async function runSnapshotCommand(env: CommanderTestEnv, args: string[] = ['snapshot']): Promise<number> {
  try {
...
/**
 * Helper to test validation status display
 * Reduces duplication in validation status tests
 */
async function expectValidationStatus(
  ...
  vi.mocked(readHistoryNote).mockResolvedValue(createMockHistory(passed));

  snapshotCommand(env.program);
  const exitCode = await runSnapshotCommand(env);
  expect(exitCode).toBe(0);

  const output = getLogOutput();
  expect(output.some(call => call.includes('Validation Status:') && call.includes(expectedStatusText))).toBe(true);
}

/**
 * Helper to test error handling
 * Reduces duplication in error handling tests
 */
async function expectSnapshotError(env: CommanderTestEnv): Promise<void> {
  snapshotCommand(env.program);
  const exitCode = await runSnapshotCommand(env);
...
describe('snapshot command', () => {
  ...
  describe('validation status display', () => {
    it('should show "Validation Status: ✅ Passed" when validation passed', async () => {
      await expectValidationStatus(env, true, 'Passed');
    ...
    it('should show "Validation Status: ❌ Failed" and suggest running state when validation failed', async () => {
      await expectValidationStatus(env, false, 'Failed');
  ...
  describe('verbose mode', () => {
    it('should show additional information in verbose mode', async () => {
      snapshotCommand(env.program);
      const exitCode = await runSnapshotCommand(env, ['snapshot', '--verbose']);
      expect(exitCode).toBe(0);
