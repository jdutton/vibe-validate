...
export function preCommitCommand(program: Command): void {
  const cmd = program
    .command('pre-commit')
    .description('Run branch sync check + validation (recommended before commit). Spawned steps run with GIT_* env vars stripped to prevent parent-repo corruption when invoked as a git hook (see docs/git-hook-safety.md).')
